...
-(BOOL) becomeFirstResponder 
{
    ...
    //if(pickerView.hidden = YES)  //change by chao.tao
...
-(void) didMoveToSuperview {
	...
	pickerView = [[NWPickerView alloc] initWithFrame:CGRectMake(0, 0, 0, 162)];
	...
	//btnpvClose
	btnpvClose= [UIButton buttonWithType:UIButtonTypeCustom];
	[btnpvClose setFrame: CGRectMake(0, 0, 768, 700)];					//fly1024
	btnpvClose.backgroundColor = [UIColor	whiteColor];

	[btnpvClose addTarget:self action:@selector(pvDismiss:) forControlEvents:UIControlEventTouchUpInside];
	[[self superview] insertSubview:btnpvClose atIndex:0];
}

- (void)pvDismiss:sender
{
...
{
	UIView *appView= [self superview];
    
    // the app window can be null when being popped off 
    // the controller stack.
	if (appView != nil) {
        CGRect windowBounds = CGRectMake(0, 768, 700, 768);	//[appView frame];		//fly1024
